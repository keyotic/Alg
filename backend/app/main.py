...
# How many interactions before the pending queue is refreshed with new algorithm results
FEED_REFRESH_EVERY = 6
...
# Tracks total interactions per user so we know when to refresh the pending queue
    ...
    if like_count >= 12 and like_ratio >= 0.7:
        return {
            ...
            "pool": [
                ...
                "\"Great opportunities come to those who stay curious.\"",
                "\"A new friendship will brighten your path.\"",
                "\"The answer you seek is already within you.\"",
                "\"Good news will arrive sooner than you expect.\"",
                "\"Your patience will be rewarded with success.\"",
                "\"Adventure can be found in ordinary moments.\"",
                "\"A small risk will lead to a valuable reward.\"",
                ...
                "\"An exciting change is on the horizon.\"",
                "\"The next step forward is simpler than it appears.\"",
                "\"Today is a good day to begin something new.\""
            ]
        }
        ...
        return {
            ...
            "skip_ratio": skip_ratio,
            "pool": [
                "\"The road less traveled will reveal something worthwhile.\"",
                "\"An old idea will find new life in your hands.\"",
                "\"Your next conversation will teach you something valuable.\"",
                "\"A hidden talent is waiting for your attention.\"",
                "\"The best decision is the one that brings peace of mind.\"",
                "\"Soon, you'll laugh about something that worries you today.\"",
                "\"A chance encounter may become an important connection.\"",
                "\"The courage to begin is all you need right now.\"",
                "\"What seems like a delay may be a blessing in disguise.\"",
                "\"A generous act will create a ripple of good fortune.\"",
                "\"Your creativity will solve a problem logic cannot.\"",
                "\"The opportunity you've been waiting for is also looking for you.\"",
                "\"An unexpected compliment will brighten your week.\"",
                "\"Success grows from consistency more than intensity.\"",
                "\"The stars favor those who make their own luck.\""
            ]
        }
        ...
        return {
            ...
            "skip_ratio": skip_ratio,
            "pool": [
                "\"A secret admirer may be closer than you think.\"",
                "\"Luck arrives when preparation meets opportunity.\"",
                "\"The heart knows a shortcut the mind has not yet discovered.\"",
                "\"A joyful surprise will find you on an ordinary day.\"",
                "\"Someone will remember your kindness long after you forget it.\"",
                "\"Romance blooms when you least expect it.\"",
                "\"A lucky break is hidden inside a challenge.\"",
                "\"Your smile will open a door that force never could.\"",
                "\"A message from afar will bring good feelings.\"",
                "\"The next wish you make deserves to be a bold one.\"",
                "\"Love grows strongest when shared freely.\"",
                "\"An unexpected gift is making its way toward you.\"",
                "\"A meaningful connection is about to deepen.\"",
                "\"A handshake today could become a celebration tomorrow.\"",
                "\"The moonlight may inspire a moment your heart will never forget.\""
            ]
        }
    else:
        return {
            ...
            "skip_ratio": skip_ratio,
            "pool": [
                "\"Someone will soon appreciate a quality in you that you rarely notice yourself.\"",
                "\"A pleasant surprise will turn an ordinary day into a memorable one.\"",
                "\"Patience today will create possibilities tomorrow.\"",
                "\"Good fortune often travels with an open mind.\"",
                "\"A small act of kindness will return to you in a larger form.\"",
                "\"Love thrives where honesty is welcomed.\"",
                "\"A chance encounter will spark a new idea.\"",
                "\"A lucky coincidence will feel anything but accidental.\"",
                "\"Someone is looking forward to hearing from you.\"",
                "\"The best surprises arrive without an announcement.\"",
                "\"A forgotten dream deserves another look.\"",
                "\"A moment of courage will change the direction of your week.\"",
                "\"A friendship is about to become more meaningful.\"",
                "\"An unexpected compliment will brighten your day.\"",
                "\"The future holds more reasons to smile than you can currently imagine.\""
            ]
        }
...
def set_pending_rows_from_items(uid: str, items: list):
    """
    Set USER_PENDING_ROWS directly from a list of items,
    skipping anything already interacted with or currently active.
    This is the ONLY place USER_PENDING_ROWS gets replaced.
    """
    ...
    for item in items:
        iid = item["item_id"]
        if iid in interacted or iid == active_id:
            continue
...
def write_user_feed_csv(uid: str):
    """
    Writes ONLY from USER_ACTIVE_ROW + USER_PENDING_ROWS.
    These are frozen between feed refreshes — only status values change.
    The item list itself only changes every FEED_REFRESH_EVERY interactions.
    """
    ...
    if active:
        ...
        lines.append(f'{pos},{active["status"]},"{item_id}","{title}"')
        pos += 1
    ...
    for row in USER_PENDING_ROWS.get(uid, []):
        ...
        item_id = str(item.get("item_id", "")).replace('"', '""')
        title   = str(item.get("title", "")).replace('"', '""')
        lines.append(f'{pos},{row["status"]},"{item_id}","{title}"')
        pos += 1
...
def rebuild_csv_state(uid: str):
    """Admin-only: full historical state for inspection."""
    rows = []
...
def feed(req: FeedRequest):
    try:
        ...
        USER_INTERACTION_COUNT.setdefault(req.user_id, 0)

        if not USER_HISTORY_ROWS[req.user_id] and req.user_id not in USER_ACTIVE_ROW:
            # Brand new session: set first item active, rest pending
            if items:
                set_active_row(req.user_id, items[0])
                set_pending_rows_from_items(req.user_id, items[1:])
            else:
                USER_PENDING_ROWS[req.user_id] = []
        # else: page was refreshed — do NOT touch USER_ACTIVE_ROW or USER_PENDING_ROWS.
        # The frontend still shows the same items; only status values may have changed.
...
def interactions(evt: Interaction):
    ...
    # Move the active item into history with its final status
    ...
    USER_INTERACTION_COUNT.setdefault(evt.user_id, 0)
...
def reset(req: ResetRequest):
    ...
    USER_INTERACTION_COUNT.pop(req.user_id, None)
...
def admin_user_detail(user_id: str):
    ...
    return {
        ...
        "interaction_count": USER_INTERACTION_COUNT.get(user_id, 0),
        "next_feed_refresh_in": FEED_REFRESH_EVERY - (USER_INTERACTION_COUNT.get(user_id, 0) % FEED_REFRESH_EVERY),
    }
...
def admin_current_user():
    if not LAST_ACTIVE_USER:
        return {
            ...
            "item_status": {},
            "interaction_count": 0,
            "next_feed_refresh_in": FEED_REFRESH_EVERY,
        }
    ...
    return {
        ...
        "interaction_count": USER_INTERACTION_COUNT.get(uid, 0),
        "next_feed_refresh_in": FEED_REFRESH_EVERY - (USER_INTERACTION_COUNT.get(uid, 0) % FEED_REFRESH_EVERY),
    }
